...
def nivel2():
    ...
    pygame.mixer.music.load("songs/nivel2.mp3")
    ...
    def savepoints():
        ...
        archivo.write(str(datetime.datetime.now()) +    ', NIVEL MEDIO ' + str(puntManzanas) + " PUNTO/S"+ "\n")
        archivo.close()
    ...
    def crear_manzana():
        ...
        while (colman, filaman) in serpiente_cuerpo or (colman, filaman) in (colcac, filcac):
    ...
    def abrir_juego():
        ...
        cuadricula = pygame.image.load("images/cuadricula-amarilla.png")
        ...
        superfondo = pygame.image.load("images/superfondo-medio.png")
        superfondopos = (0, 0)
        ...
        # cuadriculaperdida
        cuadriculaperd = pygame.image.load("images/cuadricula-amarilla_perdido.png")
        ...
        cactus_positions = []
        while len(cactus_positions) < 7:
            pos = (randint(0, 12), randint(0, 12))
            if (
                pos[1] != fila and
                pos != (col, fila) and
                pos != (colman, filaman) and
                pos not in serpiente_cuerpo and
                pos not in cactus_positions
            ):
                cactus_positions.append(pos)

        def pintar_cactus(ventana):
            global cactus
            cactus = pygame.image.load("images/cactus.png")
            for pos in cactus_positions:
                cactus_pos = cell2coord(pos[0], pos[1])
                ventana.blit(cactus, cactus_pos)
        ...
        while True:
            ...
            if (col, fila) in cactus_positions:
                # MENSAJE DESAGRADABLE
                fuente = pygame.font.Font(None, 40)
                text = "YOU HAVE DIED!    SCORE: " + str(puntManzanas)
                mensaje = fuente.render(text, 80, (121,121,121))
